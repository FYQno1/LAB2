...
void parseCode(string code[], int row)
{
    ...
    stack<int> st_switch;   //用来统计 if-else if-else
    stack<string> st_if;   //用来统计 if-else if-else
    for(int i=0; i<row; i++)
    {
        total_num += haveKeyWords(code[i]);
        
        if(code[i].find("case") != -1)
        {
            st_switch.top()++;
        }
        else if(code[i].find("switch") != -1)
        {
            st_switch.push(0);
            switch_num++;
        }
        else if(code[i].find("default") != -1)
        {
         ...
         if(code[i].find("else") != -1 && code[i].find("else if") == -1)
        {
            //cout<<"find else\n";
        ...
        else if(code[i].find("if") != -1 && code[i].find("else if") == -1)
        {
            //cout<<"if push\n";
            st_if.push("if");
        }
